# Open Devices IoT API Errors

Open Devices uses standard HTTP Errors and response codes to indicate
the success or failure of any API Call.

Codes in `1XX` range indicates information about a request.

Codes in `2XX` range indicates that the client's request was
successfully received, understood, and accepted. 

Codes in `3XX` range indicates that further action needs to be taken by
the user agent in order to fulfill the request.

Codes in `4XX` range indicates errors or the client seems to have erred.

Codes in `5XX` range indicates that the server encountered an unexpected
condition which prevented it from fulfilling the request.

References:
[HTTP Status Codes](https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml)
[HTTP Status Codes from Waldemarnt](https://github.com/waldemarnt/http-status-codes)
